...
def build_snapshot(cfg: PublishConfig) -> dict:
    ...
    if "_error" in overview:
        return {"_error": overview["_error"], "generated_at": datetime.now(timezone.utc).isoformat()}
...
def write_snapshot(cfg: PublishConfig) -> None:
    ...
    cfg.out_path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
...
def run_publish(cfg: PublishConfig, log=print) -> None:
    if not cfg.enabled:
        return
